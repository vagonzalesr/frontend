...
const { style } = global.config.style.run ? require('./style') : false;
const { script } = global.config.script.run ? require('./script') : false;
...
exports.build = series(
  ...
  global.config.sync.run && global.config.style.run ? sync.syncStartBuild : helpers.skip,
  global.config.style.run ? style.styleStart : helpers.skip,
  global.config.sync.run && global.config.style.run ? sync.syncStop : helpers.skip,
  global.config.script.run ? script.scriptStart : helpers.skip,
  global.config.sync.run && global.config.script.run ? sync.syncStop : helpers.skip,
  global.config.html.run
    && global.config.html.pug
    && (global.config.style.run ? html.htmlStart : helpers.skip)
    && (global.config.script.run ? html.htmlStart : helpers.skip),
  helpers.killNow,
);
...
exports.default = series(
  ...
  parallel(
    ...
    global.config.style.run ? style.styleListen : helpers.skip,
    global.config.script.run ? script.scriptListen : helpers.skip,
  ),
);
